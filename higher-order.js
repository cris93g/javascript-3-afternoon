...
//Code Here
let postTaxPrices= prices.map(function(pric){
  pric * 1.07;
})
...
//Code Here
let myStrongest  = monstersInYourPocket.filter(function(mon){
    return mon.CP >200;
...
let ordersTotal=orders.map(function(pr){
  let tax=pr.price * pr.tax;
   let totalPrice=tax+ pr.price;
...
let bobsTotal= purchases.filter(function(total,elem){
  console.log(total+elem.barry)
},0)
